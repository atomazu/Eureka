...
def generate_python_config_string(
    ...
    pascal_var_name = slugify(prompt_name_title).replace('_', ' ').title().replace(' ', '')
    if not pascal_var_name: pascal_var_name = "DefaultPromptName"
    if not pascal_var_name[0].isalpha() or not pascal_var_name[0].isupper():
        pascal_var_name = "P" + pascal_var_name
    pascal_case_prompt_name = pascal_var_name if pascal_var_name.lower().endswith("prompt") else pascal_var_name + "Prompt"
...
p.model = "{llm_model_override.replace('"', '\\"')}"
'''
    else:
        model_section = '''
# To use a specific LLM model for THIS prompt (overriding the global default from main config.py),
# uncomment and set the p.model line below.
# Examples:
# p.model = "phi4-reasoning"
# p.model = "qwen3:8b"
#
# If p.model is not set or is commented out, 'LLMConfig.default_model' from the main config will be used.
'''
...
""".strip()
    ...
    template_definition_str = f'p.template = f"""{escaped_template_for_codegen}"""'
...
p.anki_deck = "{anki_deck.replace('"', '\\"')}"
p.anki_ref_field = "{ref_field.replace('"', '\\"')}"
{model_section.strip()}
...
p.inputs = {{{inputs_assignment_str}}}

# The values for 'outputs' are descriptions/examples/instructions for the LLM on how to fill each output field.
p.outputs = {{{outputs_assignment_str}}}

# This will be fed to the LLM at the end.
# p.template is an f-string; p.get_inputs_json() and p.get_outputs_json() are evaluated
# when this config module is loaded, using the data from p.inputs and p.outputs.
# [[FieldName]] placeholders from p.inputs are substituted later by the main script with actual Anki note data.
{template_definition_str}

# You'll use this variable in config.py
{pascal_case_prompt_name} = p
"""
...
def main():
    ...
    if Confirm.ask("Do you want to provide a custom LLM prompt template string instead of the default?", default=False):
        CONSOLE.print(Panel(
            ...
            "You can use:\n"
            "- `{p.get_inputs_json()}` (single braces): Evaluated to a JSON string of inputs when the prompt config is loaded.\n"
            "- `{p.get_outputs_json()}` (single braces): Evaluated to a JSON string of output descriptions when prompt config is loaded.\n"
    ...
    if Confirm.ask("Proceed to save this configuration?", default=True):
        try:
            ...
            if not pascal_var_name[0].isalpha() or not pascal_var_name[0].isupper():
                pascal_var_name = "P" + pascal_var_name
            generated_variable_name = pascal_var_name if pascal_var_name.lower().endswith("prompt") else pascal_var_name + "Prompt"

            CONSOLE.print("\n[bold white on blue] Next Steps [/]")
            CONSOLE.print(f"1. Edit your main [bold]config.py[/bold].")
            CONSOLE.print(f"2. Import the generated prompt configuration, for example:")
            CONSOLE.print(f"   [cyan]from prompts.{filename_slug} import {generated_variable_name} as active_prompt_config[/cyan]")
            CONSOLE.print(f"3. Set this as the active prompt:")
            CONSOLE.print(f"   [cyan]APP_SETTINGS.active_prompt = active_prompt_config[/cyan]")
            CONSOLE.print(f"4. (Optional) If you want your main Anki settings (deck, ref_field) to automatically use those from the prompt:")
            CONSOLE.print(f"   [cyan]if APP_SETTINGS.active_prompt:[/cyan]")
            CONSOLE.print(f"   [cyan]    APP_SETTINGS.anki.deck = APP_SETTINGS.active_prompt.anki_deck[/cyan]") # type: ignore
            CONSOLE.print(f"   [cyan]    APP_SETTINGS.anki.ref_field = APP_SETTINGS.active_prompt.anki_ref_field[/cyan]") # type: ignore
            CONSOLE.print(f"5. Run your main script (e.g., [bold]python main.py[/bold]).")
